...
async createSong(createSongInput: CreateSongInput): Promise<Song> {
  ...
  async findAll(): Promise<Song[]>{
    return this.songsRepository.find({relations: ['playlists'] });
  }

  async findOne(id: number): Promise<Song> {
    return this.songsRepository.findOne({ where: { id },relations: ['playlists'] });
  }

  async update(id: number, updateSongInput: UpdateSongInput): Promise<Song> {
    await this.songsRepository.update(id, updateSongInput);
    return this.songsRepository.findOne({ where: { id },relations: ['playlists'] });  // Fetch the updated entity  
  }

  async remove(id: number) {
    const song = await this.songsRepository.findOne({ where: { id },relations: ['playlists']   });
    if (!song) {
